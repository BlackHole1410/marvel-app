    ...
    test('renders D3PieChart with label and value', () => {
        ...
        data.forEach((item) => {
            const nameLabel = screen.getByText((content, element) => {
                return element.tagName.toLowerCase() === 'text' && content.includes(item.name);
            });
            expect(nameLabel).toBeInTheDocument();
            const valueLabel = screen.getByText((content, element) => {
                return element.tagName.toLowerCase() === 'text' && content.includes(item.value.toString());
            });
